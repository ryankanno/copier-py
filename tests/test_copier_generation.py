    ...
    for dirpath, subdirs, files in os.walk(root_dir):
        ...
        for file_path in files:
            if is_absolute:
                ...
                result.append(str(Path(dirpath[len(root_dir) :]) / file_path))
    return result
...
def generate_project(tmp_path: Path, data: dict[str, str | bool]) -> Path:
...
def test_with_default_configuration(
    ...
    expected_files = (
        get_expected_baked_files(str(default_context["package_name"]))
        + EXPECTED_BAKED_AUTHORS_FILES
        + EXPECTED_BAKED_GITHUB_ACTIONS_PYPI_PUBLISH_FILES
        + EXPECTED_BAKED_GITHUB_DEPENDABOT_FILES
...
def test_with_parameterized_configuration(  # noqa: C901, PLR0912, PLR0915
    tmp_path: Path, context: dict[str, str | bool]
) -> None:
    ...
    expected_files = get_expected_baked_files(str(context["package_name"]))
...
def test_with_codecov(
    tmp_path: Path, default_context: dict[str, str | bool], codecov: bool
) -> None:
    ...
    for path in abs_baked_files:
        if "publish.yml" in path:
            with (
                ...
                if s.find(b"publish_test_pypi:") == -1 and publish_testpypi:
    ...
    for path in abs_baked_files:
        if "publish.yml" in path:
            with (
                ...
                if s.find(b"attach_to_release:") == -1 and attach_release:
                    pytest.fail("Should have attach_to_release job")
                elif (
                    s.find(b"attach_to_release:") != -1 and not attach_release
                ):
...
def test_with_python_version(
    ...
    default_context["supported_python_versions"] = "3.11, 3.12, 3.13, pypy3.11"
    ...
    for path in abs_baked_files:
        if "pyproject.toml" in path:
            with (
                ...
                if (
                    s.find(f'requires-python = ">={python_version}"'.encode())
                    == -1
                ):
                    pytest.fail(
