...
that they match. When they drift, the linter that runs locally stops
being the one that runs in CI, and the template can ship pins that
disagree with the generator that produced it.
"""
...
def copier_default(name: str) -> str:
    """The default declared for a `copier.yml` question.

    Read by regex rather than with a YAML parser, since PyYAML reaches
    this project only as a transitive dependency of copier and the tests
    should not be the reason it becomes a direct one.
    """
    ...
    assert question, f'{name} is not declared in copier.yml'
    ...
    assert default, f'{name} has no default in copier.yml'
...
def workflow_env_versions(
    ...
    Keyed by filename rather than collapsed into a set, so that a
    workflow which references the variable without declaring it stays
    visible. Such a workflow expands the reference to an empty string and
    silently installs an unpinned tool, which is the drift these tests
    exist to catch, but it would not change the set of declared values.
    """
...
def test_hook_revs_match_the_lockfile() -> None:
    ...
    binaries, which produces "All checks passed" from one and errors from
    the other.
    """
...
def test_uv_version_agrees_everywhere() -> None:
    ...
    declared = {
        'uv.lock': locked,
        'pre-commit rev': hook,
        'copier.yml': copier_default('uv_version'),
...
def test_tox_version_agrees_everywhere() -> None:
    ...
    declared = {
        'uv.lock': locked,
        'copier.yml': copier_default('tox_version'),
        **{f'workflow {name}': value for name, value in envs.items()},
    }
...
def test_template_hook_revs_match_this_repo() -> None:
    ...
    generates them, or the template's own reference code can pass here
    and fail there.
    """
    ours = hook_revs(REPO_ROOT / '.pre-commit-config.yaml')
    theirs = hook_revs(TEMPLATE_ROOT / '.pre-commit-config.yaml.jinja')
